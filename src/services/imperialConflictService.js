...
export async function distributePrizes(fastify) {
  ...
  try {
    ...
    // 4. The Imperial Ten Rewards (Top 50 individual contributors get tribe-specific badges and frames)
    try {
      const topFifty = await fastify.redis.zrevrange('giza:leaderboard:wins', 0, 49);
      if (topFifty.length > 0) {
        // Get tribe_id for each user
        const userTribesRes = await client.query('SELECT id, tribe_id FROM users WHERE id = ANY($1)', [topFifty]);
        const userTribeMap = {};
        userTribesRes.rows.forEach(r => userTribeMap[r.id] = r.tribe_id);

        // Get tribe names for the tribes
        const tribeIds = [...new Set(userTribesRes.rows.map(r => r.tribe_id).filter(Boolean))];
        const tribesRes = await client.query('SELECT id, name FROM tribes WHERE id = ANY($1)', [tribeIds]);
        const tribeNameMap = {};
        tribesRes.rows.forEach(r => tribeNameMap[r.id] = r.name);

        const TRIBE_REWARDS = {
          'Al Ahly': { badge: 'badge_gen_al_ahly', frame: 'frame_imp_al_ahly' },
          'Wydad Casablanca': { badge: 'badge_gen_wydad', frame: 'frame_imp_wydad' },
          'Raja Casablanca': { badge: 'badge_gen_raja', frame: 'frame_imp_raja' },
          'Young Africans SC': { badge: 'badge_gen_yanga', frame: 'frame_imp_yanga' },
          'Enyimba International FC': { badge: 'badge_gen_enyimba', frame: 'frame_imp_enyimba' },
          'Orlando Pirates': { badge: 'badge_gen_pirates', frame: 'frame_imp_pirates' },
          'Simba SC': { badge: 'badge_gen_simba', frame: 'frame_imp_simba' },
          'Mamelodi Sundowns': { badge: 'badge_gen_sundowns', frame: 'frame_imp_sundowns' },
          'Kaizer Chiefs': { badge: 'badge_gen_chiefs', frame: 'frame_imp_chiefs' },
          'Zamalek SC': { badge: 'badge_gen_zamalek', frame: 'frame_imp_zamalek' }
        };

        for (const userId of topFifty) {
          const tribeId = userTribeMap[userId];
          const tribeName = tribeNameMap[tribeId];
          const rewards = TRIBE_REWARDS[tribeName];

          if (rewards) {
            // Award Badge
            await client.query(
              `INSERT INTO user_badges (user_id, badge_id, awarded_at) 
               SELECT $1, id, NOW() FROM badges WHERE slug = $2 
               ON CONFLICT (user_id, badge_id) DO NOTHING`,
              [userId, rewards.badge]
            );

            // Award Frame (Collectible)
            await client.query(
              `INSERT INTO user_collectibles (id, user_id, collectible_id, acquired_at) 
               VALUES (gen_random_uuid(), $1, $2, NOW()) 
               ON CONFLICT (user_id, collectible_id) DO NOTHING`,
              [userId, rewards.frame]
            );
          }
        }
      }
    } catch (rewardErr) {
      // Don't fail the whole distribution if prestige rewards fail, but log it
      fastify.log.error(rewardErr, 'Failed to distribute Giza prestige rewards');
    }
