...
async function process50kMilestone() {
  ...
  try {
    ...
    const aresSurgeBadgeId = '770e8400-e29b-41d4-a716-446655440002';
    const eliteCenturionBadgeId = '770e8400-e29b-41d4-a716-446655440003';
    
    await client.query(`
      INSERT INTO achievements (id, name, description, criteria, tier)
      VALUES ($1, 'Ares Surge Badge', 'Awarded to users who joined during the Ares Surge (25k-30k).', '{"type": "signup_number", "range": [25001, 30000]}', 'Rare')
      ON CONFLICT (id) DO NOTHING
    `, [aresSurgeBadgeId]);

    await client.query(`
      INSERT INTO achievements (id, name, description, criteria, tier)
      VALUES ($1, 'Surge Sentinel', 'Awarded to the Elite Centurion cohort (30k-50k) during the final surge.', '{"type": "signup_number", "range": [30001, 50000]}', 'Epic')
      ON CONFLICT (id) DO NOTHING
    `, [eliteCenturionBadgeId]);
    ...
    // 2. Award 'Ares Surge Badge' to users 25,001 to 30,000
    const aresRes = await client.query(`
      INSERT INTO user_achievements (user_id, achievement_id, earned_at)
      SELECT id, $1, NOW() FROM (
        SELECT id FROM users ORDER BY created_at ASC LIMIT 5000 OFFSET 25000
      ) as cohort
      ON CONFLICT (user_id, achievement_id) DO NOTHING
    `, [aresSurgeBadgeId]);
    console.log(`AWARDED: Ares Surge Badge to ${aresRes.rowCount} users.`);

    // 3. Award 'Surge Sentinel' Badge to users 30,001 to 50,000
    const eliteRes = await client.query(`
      INSERT INTO user_achievements (user_id, achievement_id, earned_at)
      SELECT id, $1, NOW() FROM (
        SELECT id FROM users ORDER BY created_at ASC LIMIT 20000 OFFSET 30000
      ) as cohort
      ON CONFLICT (user_id, achievement_id) DO NOTHING
    `, [eliteCenturionBadgeId]);
    console.log(`AWARDED: Surge Sentinel Badge to ${eliteRes.rowCount} users.`);
    ...
    `);
    console.log(`AWARDED: 'Pre-50k Veteran' title to ${veteranRes.rowCount} users.`);
